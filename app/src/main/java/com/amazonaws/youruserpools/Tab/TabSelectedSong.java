...
import com.amazonaws.youruserpools.SlidingTabActivity;
...
public class TabSelectedSong extends Fragment {
    ...
    private void initializeData(){
        songs = new ArrayList<>();
        for(int i=0; i< config2.songs.length;i++)
        {
            songs.add(new Song(R.drawable.music,config2.songs[i]));
        }
        DBManager2.Num = config2.songs.length;
    }
    ...
    private void getData(){
        class GetData extends AsyncTask<Void,Void,String> {
            ...
            protected String doInBackground(Void... params) {
                ...
                try {
                    URL url = new URL("http://52.79.159.37/viewAllSelectedSong.php?userName="+ MainActivity.username);
    ...
    private void parseJSON(String json){
        try {
            ...
            JSONArray array = jsonObject.getJSONArray(DBManager2.TAG_JSON_ARRAY);

            config2 = new DBManager2(array.length());

            for(int i=0; i<array.length(); i++){
                JSONObject j = array.getJSONObject(i);
                DBManager2.songs[i] = getName(j);
            }
    ...
    private String getName(JSONObject j){
        String name = null;
        try {
            name = j.getString(DBManager2.TAG_Song);
        } catch (JSONException e) {
            e.printStackTrace();
        }
        return name;
